...
<div class="container mt-4">
    ...
    <form action="{{ route('questions.update', $question->id) }}" method="POST">
            ...
            <div class="input-group mb-2">
                ...
                <div class="input-group-text">
                    <input type="radio" name="correct_option" value="0" 
                        {{ ($correctOption !== null ? $correctOption == 0 : $question->options[0]->is_correct) ? 'checked' : '' }}> Correct
                </div>
            </div>
            <div class="input-group mb-2">
                ...
                <div class="input-group-text">
                    <input type="radio" name="correct_option" value="1" 
                        {{ ($correctOption !== null ? $correctOption == 1 : $question->options[1]->is_correct) ? 'checked' : '' }}> Correct
                </div>
            </div>
        @elseif($question->type === 'fill_blank' || $question->type === 'fix_answer')
            ...
            <button type="button" class="btn btn-sm btn-outline-primary" onclick="addFillBlankOption()">➕ Add Another Answer</button>
                    ...
                    <div class="input-group mb-2">
                        ...
                        <div class="input-group-text">
                            <input type="radio" name="correct_option" value="{{ $index }}" 
                                {{ ($correctOption !== null ? $correctOption == $index : $option['is_correct']) ? 'checked' : '' }}> Correct
                        </div>
                        <button type="button" class="btn btn-outline-danger" onclick="removeOption(this)">🗑️</button>
                    </div>
                @endforeach
